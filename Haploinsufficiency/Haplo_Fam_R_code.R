...
F1 <- ggplot(data=ADS, aes(x=Ortho_Age, y=freq, fill=Disease_Association)) +
  ...
  xlab(label="Gene Age MYA(Ortho_Age)") +
...
library(lattice)
setwd("/Users/mqbpqam6/Desktop/Last_Hurrah_Data_Gen/")
split <- read.csv("Last_Hurrah_Old_Genes.csv", header = T)[c(6,10)]		#Insert path to DS1
split <- na.omit(split) #Remove any rows with null assignments from the analysis
split<-subset(split, Ortho_Age!="0")
Alldisease<-subset(split, Disease_Association=="Dominant" | Disease_Association=="Recessive" | Disease_Association=="Both" | Disease_Association=="Unknown")
domdis<-subset(split, Disease_Association=="Dominant")
...
F3<-ggplot(data=lines1, aes(x=Ortho_Age, y=Proportion, group = Disease, colour = Disease)) +
  geom_line() +
  ggtitle(" ") +
  ylab(label="Proportion of total genes per Ortho_Age") +
  xlab(label=" ") +
  ...
  theme(legend.background = element_rect(fill="white", size=0.5, linetype="solid", colour ="grey86"))+
  theme(panel.background = element_rect(fill = 'transparent', colour = 'transparent'), legend.key.size = unit(.9, "cm"), legend.position = c(.90, .7),legend.title=element_text(size=9, face="bold"), legend.text = element_text(size = 9), plot.margin=unit(c(0.5,0.5,0,0.2), "cm"))
...
x11 <- split$Haplosufficiency_Rank[split$Ortho_Age=="73"]
x12 <- split$Haplosufficiency_Rank[split$Ortho_Age=="67"]
x13 <- split$Haplosufficiency_Rank[split$Ortho_Age=="43"]
x14 <- split$Haplosufficiency_Rank[split$Ortho_Age=="29"]
x15 <- split$Haplosufficiency_Rank[split$Ortho_Age=="20"]
x16 <- split$Haplosufficiency_Rank[split$Ortho_Age=="15"]
x17 <- split$Haplosufficiency_Rank[split$Ortho_Age=="9"]
x18 <- split$Haplosufficiency_Rank[split$Ortho_Age=="0"]

par(cex.lab=1, cex.axis=.8) 
boxplot(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18, names=c("796", "615", "435", "413", "351", "311","176","158","105","96","73","67","43","29","20","15","9","0"), 
        col="aliceblue", notch=TRUE, outline=FALSE, ylim=c(100,0),frame=F) +
  theme(panel.background = element_rect(fill = 'transparent', colour = 'transparent'))
title(xlab="Gene Age MYA(Ortho_Age)", ylab="Haploinsufficiency (0=HI, 100=HS)")
...
library(grid) 
setwd("/Users/mqbpqam6/Desktop/Last_Hurrah_Data_Gen/")
disess <- read.csv("Last_Hurrah_Old_Genes.csv") [c(7,8,10)]	#Path to DS1
Hap = na.omit(disess)
...
library("factoextra")
library("corrplot")
disess2 <- read.csv("Last_Hurrah_Old_Genes.csv") [c(6,7,8,10)]
...
o <- ggplot(fam1, aes(x=Family_Root, y=tots, fill = Family_Root)) + 
  geom_violin() + theme_classic() +
  ggtitle("") +
  ylab(label="Proportion of family consisting of disease genes") +
  xlab(label="Family root age") +
  #scale_fill_manual(values=group.colours)+
